  ...
  return (
    <AdminShell>
      ...
      <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-4">
        {kpis.map(([t, v, s, I]) => (
          <div key={t} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,.06)]">
            ...
            <p className="text-3xl font-semibold leading-tight">{v}</p>
            <p className="mt-1 text-sm text-emerald-600">↗ {s}</p>
          </div>
        ))}
      </div>

      <div className="mt-4">
        <AdminPanel title="Actions rapides">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {[["Examiner les demandes", "/admin/requests"], ["Gérer les restaurants", "/admin/restaurants"], ["Voir les analytics", "/admin"]].map(([t, h]) => (
              <Link key={t} href={h} className="flex items-center justify-between rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold hover:bg-slate-50">
                {t}
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-2">
        <AdminPanel title="Dernières activités" right={<button className="text-sm font-medium text-emerald-600">Voir tout</button>}>
        ...
        <AdminPanel title="État de la plateforme">
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 border-t border-slate-100 pt-4 md:grid-cols-3">
            <p className="text-sm text-slate-600">Taux d’activation<br /><span className="text-3xl font-semibold text-slate-900">96%</span></p>
            <p className="text-sm text-slate-600">Temps de réponse moyen<br /><span className="text-3xl font-semibold text-slate-900">2h 18m</span></p>
            <p className="text-sm text-slate-600">Profils complets<br /><span className="text-3xl font-semibold text-slate-900">89%</span></p>
          </div>
        </AdminPanel>
      </div>
    </AdminShell>
