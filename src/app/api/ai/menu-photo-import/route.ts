...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = [
      "Tu extrais une carte de restaurant traditionnel depuis une ou plusieurs photos.",
      ...
      "Regles strictes :",
      "- francais naturel",
      "- cible restaurant traditionnel, bistrot, brasserie ou restaurant de quartier",
      "- parcours toutes les colonnes, tous les encadres et toutes les photos",
      "- extrais chaque produit visible, sans te limiter aux premiers elements",
      ...
      "- utilise uniquement ces familles principales pour categoryName : Entrees, Plats, Desserts, Boissons, Menus",
      "- ne cree pas de categories principales Pizza, Burger, Sandwich, Tacos ou Snack",
      "- si la carte contient une sous-section comme viandes, poissons, vins, boissons fraiches ou menu enfant, garde cette information au debut de description",
      "- garde les noms commerciaux courts tels qu'ils apparaissent",
      "- les menus, formules, menu enfant, menu midi ou offres composees doivent devenir des produits importables dans la categorie Menus",
      "- mets le detail de la formule dans description quand il est lisible, par exemple entree, plat, dessert, boisson, choix inclus",
      "- si un produit demande un choix client, remplis optionsConfig.groups",
      "- exemples d'options traditionnelles : cuisson, sauce, accompagnement, boisson incluse, supplement, choix entree/plat/dessert",
      "- required true uniquement quand le choix semble obligatoire",
      "- type formula pour une formule composee",
      "- type supplement pour extras payants",
      "- type multiple_choice pour choix multiples ou supplements multiples",
      "- mets price a 0 pour les choix inclus, et le supplement en euros pour les choix payants",
      "- n'invente pas de produits ni de prix",
      "- si un meme produit apparait plusieurs fois, garde une seule ligne",
      "- limite a 80 produits",
    ].join("\n");
