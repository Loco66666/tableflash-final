...
import { Edit3, IceCreamBowl, Leaf, Sparkles, Tag, Utensils, Wine } from "lucide-react";
...
export function ProductVisual({ visual, imageUrl, alt, categoryName }: { visual: Product["visual"]; imageUrl?: string; alt?: string; categoryName?: string }) {
  ...
  return (
    <div className={`relative grid h-24 place-items-center overflow-hidden rounded-xl bg-gradient-to-br ${style.tone} sm:h-24`} aria-hidden="true">
      <span className="rounded-full bg-white/90 p-2 text-slate-700 shadow-card"><Icon className="size-5" /></span>
    </div>
  );
}
  ...
  return (
    <article className="grid grid-cols-[5.9rem_1fr_auto] items-start gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_10px_26px_rgba(15,23,42,0.08)] sm:grid-cols-[6rem_1fr_auto]">
      ...
      <div className="min-w-0">
        <h2 className="line-clamp-2 text-base font-black leading-tight tracking-[-0.02em] text-slate-950 sm:text-lg">{product.name}</h2>
        <p className="mt-0.5 truncate text-xs font-semibold uppercase tracking-wide text-slate-500">{product.categoryName}</p>
        ...
        <p className="mt-1 line-clamp-2 text-sm leading-relaxed text-slate-600 [overflow-wrap:anywhere]">{product.description}</p>
        <div className="mt-2 flex flex-wrap gap-2">
          <StatusBadge label={product.available ? "Disponible" : "Rupture"} tone={product.available ? "green" : "red"} />
          {featured ? <span className="inline-flex items-center gap-1 rounded-xl bg-emerald-50 px-2 py-1 text-xs font-bold text-emerald-700"><Tag className="size-3.5" />Suggestion</span> : null}
        </div>
      </div>
      <button type="button" onClick={() => onEdit?.(product)} className="min-h-11 self-center rounded-xl border border-emerald-100 bg-emerald-50 px-2.5 text-xs font-bold text-emerald-800 sm:px-3 sm:text-sm">
        <span className="inline-flex items-center gap-1.5"><Edit3 className="size-3.5 sm:size-4" />Modifier</span>
      </button>
    </article>
  );
}
